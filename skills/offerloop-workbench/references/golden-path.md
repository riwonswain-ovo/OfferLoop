...
> **已退役，仅供历史审计。** 旧工作台已于 2026-08-17 冻结，本文件中的部署、OAuth、数据库
> 和发布步骤不得继续执行。新工作台必须使用新的产品合同与独立应用，不得在此模板上恢复发布。

以下内容保留的是旧版本曾经采用的部署路径，不代表当前 OfferLoop 支持范围。
...
- 三个 Base 的默认视图都按“信息更新时间/创建时间/开始时间”降序，确保新增记录位于最上方；
  只改视图排序，不重排或复制记录。
- 工作台首屏只请求工作台元数据和当前 Base 的当前视图第一页；投递管理看板视图每页最多
  9 条，表格视图的三张表各自每页最多 15 条。
...
1. 授权 URL 显式传入上述四个 scope，scope 用空格分隔并正确 URL 编码。
...
1. 刷新工作台，页面应在常规网络下 10 秒内可交互；“投递进展数据”存在，看板视图第一页最多
   9 条，表格视图三张表分别读取且各自第一页最多 15 条，其他 Base/子视图可切换并翻页。
...
| 首屏约一分钟 | 首屏全量扫描多个 Base 和视图 | 元数据先行、当前视图按需读取（看板最多 9 条，表格每张表最多 15 条）；切换和翻页再加载；用 Trace 验证 |
