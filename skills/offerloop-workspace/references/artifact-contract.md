...
3. 用 `build-title` 生成确定性标题；简历深挖必须传 `--resume-version` 和
   `--target-direction`，确保标题同时包含简历版本、经历名称和目标岗位/方向。
...
- `resume-deepthink`：读取用户选定的当前简历；只有简历版本、经历名称、目标岗位/方向全部
  一致时，历史深挖文档才可用于续写或避免重复追问。目标岗位/方向不同必须创建独立文档。
