...
description: 管理 OfferLoop 的飞书求职知识库、使用指南、当前简历、训练产物、ASR 与资源入口；检查或注册企业清单、求职进展、笔面试中心和固定节点，维护目录、材料契约与工作台入口。用户说“打开/整理求职空间”“维护飞书求职首页”“登记当前简历”“检查 OfferLoop 工作台”或其他 OfferLoop Skill 需要定位飞书材料时使用。
---

# OfferLoop Workspace

把分散的求职 Base 和文档组织到固定、私有的飞书知识库入口。只维护知识库结构、首页说明、
当前简历和训练产物，不抓招聘信息、不读邮箱、不生成面试题。
...
2. 三张求职业务 Base 保持招聘数据的唯一来源；首页只登记入口，不复制 Base 记录。
3. `01｜当前简历` 可保存多个当前使用版本，每份文档标题就是唯一简历版本名；不创建历史
   简历或其他个人材料目录。
4. 简历深挖、面试准备、复盘、产品 Sense 和模拟面试按固定目录归位；不按公司建立
   顶级目录。
5. `ASR待复盘` 只保存用户上传的待处理转写；完成产物写入 `已完成复盘`，原始 ASR 不自动
   移动或删除。
6. 任何创建、移动、分享或权限变更都先列出目标并取得用户确认。
...
完整读取 `references/homepage-contract.md`。首页包含使用步骤、核心功能、数据位置、自然语言命令与常见问题；日常数据展示只在工作台和三张求职业务 Base 中进行。
...
验证知识库、首页节点、工作台入口、三个求职业务 Base 和目录是否存在。
只读检查不修复；先展示差异。
...
- 训练产物由对应 Skill 写入；本 Skill 只定位节点，不临时生成题目。
