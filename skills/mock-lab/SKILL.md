...
可选增强材料。用户可以不选择历史飞书材料并进行通用模拟，但 OfferLoop 飞书工作区本身必须已经
完成初始化且处于 `ready` 状态。
...
2. 检查 `current_resumes`、兼容目录键 `resume_deepthink`、`interview_prep` 和 `mock_lab`
   中本轮实际需要的 locator。工作区配置 schema v7、依赖或权限未就绪时路由到安装器 `--setup`，不要
   自行扩大权限。
...
经历题的参考表达只能重组用户确认事实。所有参考表达先保证内容正确，再按当前任务中用户主动提供的语言样本调整。专业题、Case 和费米题可以补充稳定专业知识、正确
...
完整结束、暂停或提前结束时自动解析 `mock_lab` locator 并写入 OfferLoop 飞书知识库的 `05｜模拟面试`；未配置或保存失败时，仍在
