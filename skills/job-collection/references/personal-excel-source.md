...
扫描前先读取已完成的 `岗位选择偏好｜<显示名>`，并核对 `用户偏好` 中的完整运行镜像。必须
取得：毕业年份、目标城市、目标/排除公司、目标/排除行业、岗位软偏好、批次时间窗，以及
`excluded_recruitment_types`。文档缺失、未完成或任一节点未确认时停止扫描并转入
`career-profile`；本 Skill 不得询问任何求职偏好，也不能把缺失当作“不限”。文档已完成而镜像
字段缺失时，按 `init-workflow.md` 从文档补齐镜像。
...
回填。等待确认或用户明确拒绝的候选不落企业表。六张企业表只允许当前 13 字段契约；不得创建、
读取、写入或验收已删除的 `编号` 等旧字段。
...
推进。写入、回读、等待确认或验收失败时保留旧游标，
并在 `last_sync_result` 记录失败阶段。

## 8. 逐来源摘要

无论是否新增，都输出：扫描窗口、源记录数、拆行后候选、硬筛淘汰、硬筛通过、岗位匹配直写、
写入前待确认、用户确认写入、用户明确不写、尚未决定、重复、新增、补全、失败、低置信度去重
冲突、旧/新游标和是否完整分页。不得用“0 条”掩盖未执行；未执行必须明确写原因。
