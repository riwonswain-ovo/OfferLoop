...
权限，并确保存在文本字段 `投递记录 ID` 和用户维护的 SingleSelect 字段 `投递简历版本`；
接管旧表时只补
缺失字段，并为每条缺失记录回填 `progress:<求职进展 record_id>`，不改写岗位、JD、投递
简历版本、日期、阶段或父级关联键。简历版本选项由用户维护，本 Skill 不读取知识库或自动
同步选项。若核心空间尚未完成初始化，本次企业同步应明确报告缺少定位并停止写入，不自行创建
另一套求职进展 Base。
