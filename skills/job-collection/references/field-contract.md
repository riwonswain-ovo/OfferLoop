...
企业记录第一次进入 `已投递` 时，当前 Agent 在同次运行中跨 Base 对账。求职进展使用两层 ID：

- `企业清单 record_id`：父级关联键，可重复；同一企业下投递多个岗位时，每个岗位各占一行；
- `投递记录 ID`：每次岗位投递的唯一键，不可重复。历史空值回填为
  `progress:<求职进展 record_id>`；自动创建的默认行使用
  `enterprise:<企业清单 record_id>:default`。
...
- `最近完成节点` 记录已经可靠完成的最晚节点。`当前阶段`、`下一环节`、`流程结果`、`当前状态`
  仅为 schema v5 过渡兼容字段；新逻辑不得把它们当作当前状态真源，也不得据此覆盖已有
  `进展状态`；
- 新建：进展状态=`待反馈`、最近完成节点=`投递完成`、公司=来源公司，投递岗位和岗位 JD
  为空，投递日期为本次首次变更日期，并逐值复制企业清单的公告链接和投递链接；
- 简历选择属于具体简历任务的上下文，不写入业务 Base；
- 求职进展不保留“原招聘信息”字段；每次对账都刷新公告链接与投递链接，使其与企业清单一致；
- 历史迁移日期无法可靠恢复时，投递日期留空；
- 父级下完全没有进展时才创建一条默认行；已有多条岗位进展时逐条更新，不新增默认行；
- 重复事件不得覆盖用户填写的投递岗位、岗位 JD、投递日期、`进展状态`或`最近完成节点`；
- 已有进展只刷新公司、公告链接、投递链接、父级关联键，并补齐缺失的 `投递记录 ID`；
  不得写入已删除的 `原招聘信息` 字段；
