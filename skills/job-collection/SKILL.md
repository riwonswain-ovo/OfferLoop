...
| 场景 | 必须读取 | 缺失时 |
|---|---|---|
| 招聘信息同步 | 已完成的 `岗位选择偏好｜<显示名>`；信息源登记 | 唯一匹配自动读取；缺失、空白或 `incomplete` 时转入 `career-profile` 的固定流程，不得自行询问任何求职偏好 |
...
首次使用、profile 未选定或核心空间缺失时，运行安装器的 `--setup` 或 `--verify` 管理流程；不要调用一次性安装 Skill。
...
3. 优先读取已完成的 `岗位选择偏好｜<显示名>`，再读取现有 `用户偏好` 作为运行镜像。文档由
   `career-profile` 唯一维护；任一固定节点缺失或未确认时暂停接管并转入该 Skill，
   `job-collection` 不得询问毕业年份、专业、岗位经历、岗位方向、城市、招聘类型、行业或企业偏好。
...
1. 读取 `references/init-workflow.md`；从已完成的岗位选择偏好文档取得全部求职偏好并镜像到
   `用户偏好`。文档缺失或未完成时转入 `career-profile`；本流程只补飞书用户标识和其他技术配置，
   不提问任何求职偏好。
...
3. 先读取已完成的 `岗位选择偏好｜<显示名>`，再核对并更新 `用户偏好` 运行镜像。文档是人工
   可读唯一真源；Base 与文档冲突时以已确认文档覆盖镜像，不再重复询问用户。禁止硬编码城市、
   行业、岗位、届次和排除项。
...
4. 固定执行硬筛：排除公司 → 目标城市 → 毕业年份 → 批次时间窗 → 目标行业 → 排除行业 →
   ...
   岗位选择偏好文档缺失、未完成或任一筛选节点未确认时，在扫描来源前转入 `career-profile`；
   不得由本 Skill 补问，也不得把缺失当作无限制。
...
`03｜定制简历` 中的标准文档标题完全一致。本 Skill 是招聘信息
...
3. 父级下完全没有进展时才创建默认进展，`投递记录 ID=enterprise:<企业记录ID>:default`：
   岗位、岗位 JD 和投递简历版本为空；正常新事件填首次投递日期，无法可靠
   恢复日期的历史记录保持空白。
4. 父级下已有一条或多条进展时逐条刷新公司、公告链接、投递链接、父级关联键，并补齐缺失的
   `投递记录 ID`；两个链接与企业清单保持一致；不得覆盖用户填写的岗位、岗位 JD、投递
   简历版本、已有
   投递日期或更后的面试阶段。
...
工作台未选择、未部署或不可用时无需处理。已部署的工作台会按需读取 Base，
...
发送时复用本轮 `run_id` 生成 idempotency key。通知失败不得回滚 Base 写入、进展对账或已满足
