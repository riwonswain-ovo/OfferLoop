...
description: |
  ...
  当用户提到同步招聘表、导入招聘信息、更新求职清单、检查信息源更新、查询待确认/感兴趣/已投递/已拒绝清单，或提供 feishu.cn/base、larksuite.com/base、docs.qq.com/smartsheet 链接时触发。
---
...
3. 首次缺失时创建一条进展：岗位和岗位 JD 为空；正常新事件填首次投递日期，无法可靠
   恢复日期的历史记录保持空白。
4. 已有进展只刷新公司、原招聘信息和技术 ID；不得覆盖用户填写的岗位、岗位 JD、已有
   投递日期或更后的面试阶段。
5. 重复运行结果必须是同一条记录。发现同一技术 ID 对应多条进展时停止自动写入并报告。

当 Codex 在本次运行中把任一企业主记录更新为 `已投递` 时，必须不等待下一次任务，立即：
