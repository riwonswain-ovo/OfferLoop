...
`02｜简历合集` 中的文档标题完全一致，例如 `互联网产品经理岗 - 简历`。本 Skill 是招聘信息
...
视为后续运行的持续授权；后续增量同步直接向该固定接收方发送，不得每轮重复要求用户确认。
配置缺失或停用时不发送。实际发送前完整读取 `lark-im` Skill。

仅在初始化或增量同步这类产生业务写入的运行结束后发送固定格式汇总；清单查询、dry-run 和纯结构
审计默认不发送。消息必须列出本轮全部有效新增记录，不得只报告新增数量或省略公司名称。按以下
Markdown 模板和区块顺序生成：
...
### 来源统计
- **来源名**：窗口；候选 N / 硬筛后 N / 重复 N / 新增 N / 补全 N / 失败 N；游标变化。

### 验收
- 主子表写入与回读、既有记录一致性、workflow、求职进展对账和工作台状态。
```

逐公司条目遵循以下规则：

1. 使用本轮最终写入主表的记录生成，一条新增记录至少对应一个条目；同公司同批次且投递入口相同
   时才合并岗位，禁止因公司重名吞掉不同批次、项目或入口。
2. 固定写出公司、招聘批次、岗位摘要、城市、截止时间和可点击入口。优先使用投递链接并标为
   `投递入口`；没有投递链接时使用公告链接并标为 `招聘公告`。
3. 岗位过长时压缩为忠于原字段的岗位类别摘要，不编造岗位；城市过长时保留命中用户偏好的城市并
   用“等”收束。截止时间为空时写“尽快投递”或来源中的等价值。
4. 本轮没有新增时，在“新增招聘信息（按公司）”下写 `- 无新增。`，但仍保留来源统计与验收。
5. 存在低置信度冲突时只在“待确认”中写公司和冲突类型，不发送敏感详情；没有时固定写 `- 无。`。

只发送通过用户偏好硬筛并已写入目标 Base 的新增公司清单，不发送未筛选来源表内容、凭证或完整
技术错误。消息超过平台限制时仅在公司条目边界拆分，并使用同一 `run_id` 加分片序号生成幂等键；
不得通过删掉公司条目缩短消息。
