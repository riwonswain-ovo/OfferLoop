...
description: 介绍 OfferLoop 的十个 Skill，并初始化必需的私有飞书知识库、三张业务 Base、飞书身份和公共配置。适用于“安装/初始化 OfferLoop”“第一次使用”“介绍十个 Skill”“检查环境或配置”“修复任一 OfferLoop Skill”“搭建完整 OfferLoop”；所有能力共享知识库核心空间，飞书工作台仅在用户选择 offerloop-workbench 时按需部署。
---
...
用户表示刚安装、第一次使用、不了解各 Skill，或明确要求介绍十个 Skill 时，完整读取
`references/welcome.md`，先展示十项能力、两个常用闭环和隐私边界，再询问本次要启用哪项能力。
...
本仓库包含四个核心与业务 Skill、一个可选工作台 Skill 和五个求职训练 Skill。任一能力都先
