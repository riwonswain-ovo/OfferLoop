...
`--enable-coaching` 只在用户明确确认后执行。它保留既有公共配置和兼容 locator，追加或迁移
`artifact_storage` 并升级为 schema v4；不创建飞书节点。训练目录在首次运行对应 Skill 时
由 `offerloop-workspace` 经用户确认懒创建。用户拒绝升级时，原有能力继续可用。
