...
本接入指南采用即时体验优先、业务能力渐进配置。先确认十二个 OfferLoop Skill 已安装在同一个
...
选择当前能力；只介绍与当前目标相关的 Skill。用户明确要求理解十二个 Skill 时，才展示完整能力
