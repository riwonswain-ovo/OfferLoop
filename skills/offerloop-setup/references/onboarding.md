...
完整模式不执行全局画像门禁。每个 Skill 只收集当前任务必需信息；`job-collection` 以 Base
「用户偏好」为唯一真源。检测到旧 `single` 配置时先升级到完整模式，不继续提供 Chat-only 入口。
...
旧工作区的画像和能力训练目录迁入 `99｜历史归档` 只读保留；新工作区没有历史内容时不创建归档目录。
