---
name: interview-prep
description: 为已经收到或即将参加各行业、各职能岗位面试的用户生成当前轮次的针对性准备文档。先确认公司、岗位、完整 JD、面试轮次和个人材料；自我介绍必须在隔离阶段只读取专用方法 reference 生成并冻结，再读取其他 reference 建立岗位画像、研究公司与高相关面经，输出公司与岗位认知、专业知识补齐，以及严格贴合目标公司的 15 道正式题和较完整参考答案。产品经理及以产品判断为核心的复合岗位必须覆盖行为动机题、工作职能题、开放业务题、产品实战题和费米估算题；面经原题优先，缺口再按母题动态生成。用户说“收到面试邀请了”“帮我准备某公司某岗位一面/二面/终面/HR 面”“根据 JD 准备面试”“结合我的经历做面试准备”时使用；简历项目深挖交给 experience-deepthink，本 Skill 不另建经历战斗卡，不进入一题一答模拟，也不处理笔试。
---
...
无论用户只要自我介绍，还是要生成完整面试准备文档，都必须先执行阶段 A。除当前 `SKILL.md`
和运行模式检查外，在阶段 A 开始前不得读取本 Skill 的其他 reference。

阶段 A 的顺序固定为：

1. 收集公司、岗位、JD、当前轮次、用户确认的简历与经历，以及当前任务中已经核验的公司事实；
2. 只读取 `references/self-introduction-method.md` 作为写作方法来源；
3. 生成自我介绍，仅按该文件的“交付前检查”检查，然后冻结结果。

上述公司、JD、简历、经历和已核验事实属于内容证据，不属于写作方法 reference。阶段 A 不得
读取或套用 `preparation-template.md`、`quality-gates.md`、面经检索方法、轮次规则、通用回答框架、
岗位适配文件、问题预测文件、岗位指南或 `voice-contract.md`；也不得先读完这些文件再声称只以
`self-introduction-method.md` 为准。事实不足时向用户确认；如确需核验一项公司事实，只做最小
事实检索，不读取其他面试准备方法文件。

若当前任务中已有用户明确确认的自我介绍，以确认版为底稿，只做用户要求的修改和必要的事实、
公司或岗位适配；除非用户明确要求推倒重写，不得因“重新调用 Skill”另生成一套表达。

只生成、改写或检查自我介绍时，阶段 A 完成后立即停止，不加载任何其他 reference。

### 阶段 B｜生成完整文档的其余部分

只有在自我介绍已生成并冻结后，完整面试准备文档才进入阶段 B，并按需完整读取：
...
阶段 B 的 reference 用于公司认知、专业知识、十五道题、参考答案和反问。组装完整文档时，必须
原样插入阶段 A 已冻结的自我介绍；不得让阶段 B 的模板、质量门禁、语言样本或岗位规则改写其
结构、篇幅、语气和选材。若后续研究发现自我介绍中存在实质性事实错误，只把修正后的事实带回
阶段 A，重新读取 `self-introduction-method.md` 后单独修正并再次冻结，不得直接在阶段 B 改写。
...
按照 `references/preparation-template.md` 组装完整文档，只包含：

1. 阶段 A 已隔离生成并冻结的 1—2 分钟人岗匹配式自我介绍；
...
自我介绍只能在阶段 A 生成、改写或检查，并以 `references/self-introduction-method.md` 作为结构、
证据选择、公司化改写、时长和追问钩子的单一方法来源；进入阶段 B 后，通用回答规范不得覆盖或
回写已冻结结果。
十五道参考答案遵守 `references/answer-logic.md`。答案必须同时体现岗位场景理解和用户真实经历；
个人材料没有证据时标记缺口并给出诚实口径，不生成万能答案或伪个人化内容。技术、
案例、销售、设计、研究和专业判断题使用各自适合的内部推理骨架，但对外仍保持结论先行。归纳/
演绎标签和内部逻辑检查默认不展示；用户看到记忆链和自然口语答案即可。

1—2 分钟自我介绍始终是正式版本。只有群面/短时开场、用户明确需要或邀请信息显示时间受限时，
才额外提供 30 秒压缩版。候选人反问按已确认的面试官性质和本轮证据适配；不得仅凭“一面/
二面”推断对方层级。

阶段 A 只依据当时已确认的公司、JD、轮次和面试性质事实调整自我介绍；阶段 B 再根据岗位画像、
轮次和面试性质调整十五道题、回答深度和反问，不回写自我介绍。默认轮次模型只是启发式规则；
公司官方信息、用户明确说明或高相关面经提供更可靠证据时，以证据为准并说明依据。
