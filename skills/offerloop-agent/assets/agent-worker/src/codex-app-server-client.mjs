...
class CodexAppServerClient {
  constructor({
    ...
    this.threadsAwaitingFirstMessage = new Set();
  ...
  async createThread(title = 'OfferLoop 新对话') {
    const threadId = await this.ensureThread(undefined, title);
    this.threadsAwaitingFirstMessage.add(threadId);
    return threadId;
  }

  async createVisibleThread({ onUpdate }) {
    const execution = await this.runTurn({
      confirmed: false,
      message:
        'OfferLoop 工作台已创建这个对话。请只回复“已连接”，不要读取或修改任何文件。',
      onUpdate: (update) => {
        const visibleUpdate = {};
        if (typeof update.progress === 'string') {
          visibleUpdate.progress = update.progress;
        }
        if (typeof update.sessionId === 'string') {
          visibleUpdate.sessionId = update.sessionId;
        }
        onUpdate(visibleUpdate);
      },
      route: 'chat',
    });
    const completion = execution.completion.then(async (outcome) => {
      if (outcome.ok) {
        await this.request('thread/name/set', {
          name: 'OfferLoop 新对话',
          threadId: execution.threadId,
        }).catch(() => undefined);
        this.threadsAwaitingFirstMessage.add(execution.threadId);
      }
      return outcome;
    });
    return { completion, threadId: execution.threadId };
  }
  ...
  async runTurn({
    ...
    if (this.threadsAwaitingFirstMessage.has(threadId)) {
      ...
      this.threadsAwaitingFirstMessage.delete(threadId);
    }
