...
function createWorkerClient({ apiKey, baseUrl, ownerId, fetchImpl = fetch }) {
  if (!apiKey) {
    throw new Error('OfferLoop Workbench API key is required');
  }
  if (!ownerId) {
    throw new Error('OfferLoop Feishu user ID is required');
  }
  ...
  return {
    poll(body) {
      return request('/openapi/agent-worker/poll', 'POST', {
        ...body,
        ownerId,
      });
    },
