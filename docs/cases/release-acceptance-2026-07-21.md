...
- 求职进展同步服务测试：20 项通过。
- 三 Agent 隔离 HOME 冷安装：Codex、Claude Code、Hermes 均安装四个 Skill，重复执行返回 `already_installed`。
...
CI 已配置 Ubuntu、macOS 和 Windows 的三 Agent 隔离 HOME 冷安装。本地验收只代表当前 macOS 环境；三系统结论需以 GitHub Actions 实际运行结果为准。
