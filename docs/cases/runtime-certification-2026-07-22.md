...
- WorkBuddy：在真实腾讯 WorkBuddy 5.1.7 中，技能管理页发现四个 OfferLoop Skill；新建空白工作区后，运行时实际加载 `offerloop-setup`，解析到 WorkBuddy 用户安装目录，并执行 `collection → reminder → workspace → full` 的只读离线预检。初验发现 WorkBuddy 管理副本停留在 `lark-cli 1.0.60`，且会遮蔽宿主的兼容版本；普通 `lark-cli update` 只升级了宿主 npm 副本。将官方 `@larksuite/cli` 明确更新到 WorkBuddy 管理前缀后，新建真实任务复验得到 `lark-cli 1.0.76`、3 个可发现 profile，`collection` 返回 `schema_version: 1` 和 `overall_status: unverified`，已无 `blocked`/`needs_action`；仅 `online.permissions` 与已登记通知保持 `unverified`。外部飞书 Skill 已从 WorkBuddy 飞书连接器中发现，未访问飞书、邮箱或浏览器，也未创建或修改业务资源。
...
Codex、Claude Code、Hermes 与腾讯 WorkBuddy 均达到真实加载和安装目录只读执行门禁。Claude Code 的认证覆盖 CC Switch 当前启用的第三方模型；WorkBuddy 的真实产品契约是用户目录中的 AgentSkills `SKILL.md`，不需要 `skill.yml` 包装层，其本地 `lark-cli` 与 profile 门禁现已通过。所有线上和写入能力继续分别认证。
