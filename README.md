...
- 修复腾讯 WorkBuddy 管理的 `lark-cli` 停留在旧版本时的升级指引：预检现在会给出针对该 Agent 的恢复命令，而不会建议复制可执行文件或覆盖 profile。
