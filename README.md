...
- 腾讯 Smartsheet 优先使用腾讯文档官方 MCP 的本地 Personal Token；Token 不进入对话、仓库或
  业务 Base。MCP 不可用时回退到用户已登录浏览器中的导出、复制或断点分批读取。
...
5. 首次完整扫描来源，执行批次拆分、偏好筛选和跨来源去重。腾讯 MCP 来源按
   `offset/next` 分页到 `has_more=false`，并核对唯一记录数等于稳定的来源总数后才算完成。
...
知识库固定保存于：
00｜OfferLoop 使用指南、01｜核心求职数据（三张 Base）、02｜简历合集、03｜经历深挖、
